 ...
 async function getRestaurentDetailInformation(id) {
    ...
    try{
       ...
       let response = await fetch(url,{
        ...
        .then(data =>{ 
            //TODO : Better to check validity of all data
            let defaultRestaurent = {
                name : data.name,
                imgURL : data.photos != undefined ? data.photos[0].photo.url : "Content/Images/no_image.jpg",
                address : data.location.address,
                delivery:"",//TODO Implement it too
...
const getInitialResturants = result => {
    ...
    arrResturantResult.forEach((element,index) => {
        ...
        console.log(typeof(element.restaurant.photos));
        let defaultRestaurent = {
